...
- Create a `config.js` file within the `src/` dir
- `config.js` file must expose an `API_TOKEN` key, e.g. (`module.exports = { API_KEY:"<YOUR_API_KEY>" }`)
...
Runs the app transpiling scss task.<br>

### `npm style:watch`

Watches for any scss changes in your project while developing.<br>
...
`public` dir to be deployed

## Improvements

- Use a flux pattern to maintain the App state (`redux` branch has been created) to avoid React lifecycle and async issues.
- Create more modules within `Weather.js` component
- Add client-side data validation and more error messages for the client
